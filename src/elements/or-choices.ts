...
export class OrChoices {
	...
	private initChoices() {
		...
		if (this.oChoices) this.config.choices = this.oChoices;
		...
		if (this.choicesInstance && this.oChoices) {
			this.choicesInstance.setChoices(
				this.oChoices,
				this.oChoicesValueName,
				this.oChoicesLabelName,
				true
			);
		}
	}
	private buildPlaceholder() {
		if (this.oPlaceholder && this.oPlaceholderValue && !this.oMultiple) {
			...
			this.choicesElement.insertBefore(
				node,
				this.choicesElement.firstChild
			);
		}
	}
	...
	//--- GENERATED CODE ---
	@bindable({ defaultBindingMode: bindingMode.twoWay })
	public oAddItemFilter:
		| string
		| RegExp
		| Choicesns.Types.filterFunction
		| null;
	...
	private initialiseProperties() {
		...
		if (
			this.oCallbackOnCreateTemplates !== undefined &&
			this.oCallbackOnCreateTemplates !== null
		) {
			this.config.callbackOnCreateTemplates = this.oCallbackOnCreateTemplates;
		}
		if (
			this.oCallbackOnInit !== undefined &&
			this.oCallbackOnInit !== null
		) {
		...
		if (
			this.oCustomAddItemText !== undefined &&
			this.oCustomAddItemText !== null
		) {
		...
		if (
			this.oDuplicateItemsAllowed !== undefined &&
			this.oDuplicateItemsAllowed !== null
		) {
		...
		if (
			this.oItemSelectText !== undefined &&
			this.oItemSelectText !== null
		) {
		...
		if (
			this.oPlaceholderValue !== undefined &&
			this.oPlaceholderValue !== null
		) {
		...
		if (
			this.oRemoveItemButton !== undefined &&
			this.oRemoveItemButton !== null
		) {
		...
		if (
			this.oRenderChoiceLimit !== undefined &&
			this.oRenderChoiceLimit !== null
		) {
			this.config.renderChoiceLimit = this.oRenderChoiceLimit;
		}
		if (
			this.oRenderSelectedChoices !== undefined &&
			this.oRenderSelectedChoices !== null
		) {
			this.config.renderSelectedChoices = this.oRenderSelectedChoices;
		}
		if (
			this.oResetScrollPosition !== undefined &&
			this.oResetScrollPosition !== null
		) {
		...
		if (
			this.oSearchPlaceholderValue !== undefined &&
			this.oSearchPlaceholderValue !== null
		) {
			this.config.searchPlaceholderValue = this.oSearchPlaceholderValue;
		}
		if (
			this.oSearchResultLimit !== undefined &&
			this.oSearchResultLimit !== null
		) {
		...
		if (
			this.oShouldSortItems !== undefined &&
			this.oShouldSortItems !== null
		) {
		...
		if (
			this.oUniqueItemText !== undefined &&
			this.oUniqueItemText !== null
		) {
